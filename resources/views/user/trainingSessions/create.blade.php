...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.trainingSession.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.training-sessions.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('scheduled') ? 'is-invalid' : '' }}" type="text" name="scheduled" id="scheduled" value="{{ old('scheduled') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('attendance_type') ? 'is-invalid' : '' }}" name="attendance_type" id="attendance_type" required>
                    ...
                    @foreach(App\Models\TrainingSession::ATTENDANCE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('attendance_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
