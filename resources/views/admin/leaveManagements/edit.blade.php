...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.leaveManagement.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.leave-managements.update", [$leaveManagement->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $leaveManagement->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $employee }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_start') ? 'is-invalid' : '' }}" type="text" name="date_start" id="date_start" value="{{ old('date_start', $leaveManagement->date_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_start') ? 'is-invalid' : '' }}" type="text" name="end_start" id="end_start" value="{{ old('end_start', $leaveManagement->end_start) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('details') ? 'is-invalid' : '' }}" name="details" id="details">{{ old('details', $leaveManagement->details) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\LeaveManagement::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $leaveManagement->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_type') ? 'is-invalid' : '' }}" name="leave_type_id" id="leave_type_id" required>
                    @foreach($leave_types as $id => $leave_type)
                        <option value="{{ $id }}" {{ (old('leave_type_id') ? old('leave_type_id') : $leaveManagement->leave_type->id ?? '') == $id ? 'selected' : '' }}>{{ $leave_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_periode') ? 'is-invalid' : '' }}" name="leave_periode_id" id="leave_periode_id" required>
                    @foreach($leave_periodes as $id => $leave_periode)
                        <option value="{{ $id }}" {{ (old('leave_periode_id') ? old('leave_periode_id') : $leaveManagement->leave_periode->id ?? '') == $id ? 'selected' : '' }}>{{ $leave_periode }}</option>
                    @endforeach
                </select>
