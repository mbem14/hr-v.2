    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('coordinator') ? 'is-invalid' : '' }}" name="coordinator_id" id="coordinator_id">
                    @foreach($coordinators as $id => $coordinator)
                        <option value="{{ $id }}" {{ old('coordinator_id') == $id ? 'selected' : '' }}>{{ $coordinator }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('trainer_info') ? 'is-invalid' : '' }}" name="trainer_info" id="trainer_info">{{ old('trainer_info') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cost') ? 'is-invalid' : '' }}" type="number" name="cost" id="cost" value="{{ old('cost', '0.00') }}" step="0.01">
                @if($errors->has('cost'))
                    <div class="invalid-feedback">
                        {{ $errors->first('cost') }}
                    </div>
