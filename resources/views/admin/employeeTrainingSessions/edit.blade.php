...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.employeeTrainingSession.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.employee-training-sessions.update", [$employeeTrainingSession->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employees') ? 'is-invalid' : '' }}" name="employees[]" id="employees" multiple required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $id }}" {{ (in_array($id, old('employees', [])) || $employeeTrainingSession->employees->contains($id)) ? 'selected' : '' }}>{{ $employee }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('training_session') ? 'is-invalid' : '' }}" name="training_session_id" id="training_session_id" required>
                    @foreach($training_sessions as $id => $training_session)
                        <option value="{{ $id }}" {{ (old('training_session_id') ? old('training_session_id') : $employeeTrainingSession->training_session->id ?? '') == $id ? 'selected' : '' }}>{{ $training_session }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('feedback') ? 'is-invalid' : '' }}" name="feedback" id="feedback">{{ old('feedback', $employeeTrainingSession->feedback) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\EmployeeTrainingSession::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $employeeTrainingSession->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
