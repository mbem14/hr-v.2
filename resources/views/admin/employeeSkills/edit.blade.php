    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employee-skills.update", [$employeeSkill->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('skill') ? 'is-invalid' : '' }}" type="text" name="skill" id="skill" value="{{ old('skill', $employeeSkill->skill) }}" required>
                @if($errors->has('skill'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('details') ? 'is-invalid' : '' }}" name="details" id="details">{{ old('details', $employeeSkill->details) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $employeeSkill->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $employee }}</option>
                    @endforeach
                </select>
