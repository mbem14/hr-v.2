...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.employeeLanguage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.employee-languages.update", [$employeeLanguage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employee') ? 'is-invalid' : '' }}" name="employee_id" id="employee_id" required>
                    @foreach($employees as $id => $employee)
                        <option value="{{ $id }}" {{ (old('employee_id') ? old('employee_id') : $employeeLanguage->employee->id ?? '') == $id ? 'selected' : '' }}>{{ $employee }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('languages') ? 'is-invalid' : '' }}" name="languages_id" id="languages_id" required>
                    @foreach($languages as $id => $languages)
                        <option value="{{ $id }}" {{ (old('languages_id') ? old('languages_id') : $employeeLanguage->languages->id ?? '') == $id ? 'selected' : '' }}>{{ $languages }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('reading') ? 'is-invalid' : '' }}" name="reading" id="reading" required>
                    ...
                    @foreach(App\Models\EmployeeLanguage::READING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('reading', $employeeLanguage->reading) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('speaking') ? 'is-invalid' : '' }}" name="speaking" id="speaking" required>
                    ...
                    @foreach(App\Models\EmployeeLanguage::SPEAKING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('speaking', $employeeLanguage->speaking) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('writing') ? 'is-invalid' : '' }}" name="writing" id="writing" required>
                    ...
                    @foreach(App\Models\EmployeeLanguage::WRITING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('writing', $employeeLanguage->writing) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('listening') ? 'is-invalid' : '' }}" name="listening" id="listening" required>
                    ...
                    @foreach(App\Models\EmployeeLanguage::LISTENING_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('listening', $employeeLanguage->listening) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
