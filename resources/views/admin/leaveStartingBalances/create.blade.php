...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.leaveStartingBalance.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.leave-starting-balances.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('leave_period') ? 'is-invalid' : '' }}" name="leave_period_id" id="leave_period_id" required>
                    @foreach($leave_periods as $id => $leave_period)
                        <option value="{{ $id }}" {{ old('leave_period_id') == $id ? 'selected' : '' }}>{{ $leave_period }}</option>
                    @endforeach
                </select>
